...
//
// ⚠️ 这是 src/tier-slider.js 的浏览器镜像 —— 后端在保存配置时用它
//    **重新权威换算**档位与概率，前端这份只负责"界面即时反馈"。
//    两边逻辑必须保持完全一致，否则"界面显示的档位"和"实际生效的档位"会对不上。
//    （ui/app.js 是普通 script 非 ES module，无法 import src/，故镜像于此。）
//
// 滑条分段（长度由用户指定）：
//
//   0 ──── 10 ──── 20 ───────────────── 90 ──── 100
